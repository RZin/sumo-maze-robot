...
class Driver:
    ...
    def __init__(self, rate=10):
        ...
        # set_initial pose to 0 0 0
        pose = Pose2D(0.001, 0, 0)
        self.set_pose_pub = rospy.Publisher('/set_pose', Pose2D,
                                            queue_size=100)
    ...
    def pose_cb(self, msgIn):
        ...
        # update velocities
        self.v = (msgIn.x - self.x) * self.rate_val / np.cos(self.theta)
        # self.v = (msgIn.y - self.y)*self.rate_val/np.sin(self.theta)
        self.w = (msgIn.theta - self.theta) * self.rate_val
